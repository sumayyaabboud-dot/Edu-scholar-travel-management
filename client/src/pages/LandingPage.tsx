...
export default function LandingPage() {
  ...
  return (
    <div className="theme-bright" style={{ background: 'linear-gradient(180deg,#EFFBFD,#FFFFFF)', color: '#0B1130', position: 'relative' }}>
      {/* NAV */}
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 48px', position: 'relative', zIndex: 3 }}>
      ...
      {/* HERO */}
      <div style={{ position: 'relative', overflow: 'hidden', padding: '56px 48px 90px' }}>
        <svg width="480" height="260" viewBox="0 0 480 260" style={{ position: 'absolute', right: 40, bottom: -10, opacity: 0.4 }}>
          <path d="M10 230 Q 160 40 460 70" fill="none" stroke="#22D3EE" strokeWidth="2" strokeDasharray="7 7" />
          <circle cx="10" cy="230" r="5" fill="#22C55E" />
          <g transform="translate(452,64) rotate(-18)"><path d="M0 0 L14 5 L0 10 L3 5 Z" fill="#FF6B4A" /></g>
        </svg>

        <div className="shell" style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: 700 }}>
            <h1 style={{ fontSize: 'clamp(2.5rem,4vw,3.7rem)', lineHeight: 1.06, color: '#0B1130', fontWeight: 700 }}>
              One pipeline for every seat, from application to boarding pass.
            </h1>
            <p style={{ marginTop: 22, fontSize: 16.5, color: 'rgba(11,17,48,.68)', maxWidth: 490 }}>
            ...
            <div style={{ marginTop: 34, display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              ...
              <button className="btn large" style={{ background: 'rgba(11,17,48,.05)', color: '#0B1130', border: '1px solid rgba(11,17,48,.15)' }} onClick={scrollToHow}>See how it works</button>
            </div>
          </div>

          <div className="card" style={{ marginTop: 40, padding: 0, overflow: 'hidden', boxShadow: '0 0 0 1px rgba(34,211,238,.15), 0 20px 50px -16px rgba(34,211,238,.4), 0 6px 24px -8px rgba(11,17,48,.1)' }}>
            <img src={heroImg} alt="Your journey starts by scholarship reward — Global Airways, 2026-2027" style={{ width: '100%', height: 'auto', display: 'block' }} />
          </div>
        </div>
      </div>
      ...
      <div className="shell" id="story" style={{ padding: '64px 48px 10px' }}>
        <div className="card" style={{ padding: 0, overflow: 'hidden', borderLeft: '4px solid #22D3EE' }}>
          <div className="grid-2" style={{ padding: '38px 44px', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 28, alignItems: 'flex-start' }}>
      ...
      <div className="shell" id="how" style={{ padding: '70px 48px' }}>
        ...
        <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 14, marginTop: 36 }}>
